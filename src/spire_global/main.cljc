(ns spire-global.main
  "Kotodama WASM entrypoint for the Spire_global clean-room actor (L4) — Clojure port.
  ...
  facts (`spire_global.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "spire_global")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"spire_global.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "spire_global-compat" :tier tier :entities entities} 200])
...
(defn start! [] :spire_global-compat/ready)
